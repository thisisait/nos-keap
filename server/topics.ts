...
async function runOnce(opts: { reset?: boolean }): Promise<TopicRunResult> {
  ...
  const model = db.dominantObjectModel();
  if (!model) return done({ ok: false, skipped: 'no-vectors', k: 0, n: 0, moved: 0, born: [], retired: [] });

  let stored = db.listTopicClusters();
  // ── 1) Model guard (decision #5): a dominant-model flip is a sanctioned
  //       identity break — old-space centroids are meaningless in the new one.
  let reset = opts.reset === true;
  if (stored.length > 0 && stored[0].model !== model) reset = true;
  if (reset) stored = [];
  ...
  const allObjects = db.getObjects('', true);
  ...
  // ── 7) Labels + churn (decisions #6, #7).
  const docsByCluster = new Map<string, LabelDoc[]>();
  for (const w of working) {
    const docs: LabelDoc[] = [];
    for (const oid of membersByTopic.get(w.id) ?? []) {
  ...
  const labels = cTfIdfLabels(docsByCluster);
...
function computeBirthTheta(
  memberIds: string[],
  topicId: string,
  objectsById: Map<string, { links?: unknown[] }>,
): number {
...
export function topicsStale(): boolean {
  ...
  if (stored.length === 0) return true; // never clustered but vectors exist
  if (stored[0].model !== model) return true; // dominant-model flip
  const { count } = db.objectVectorStats(model);
